...
/**
 * A panel with a specific number of columns where label-component pairs can be 
 * added to. It also offers parameters to configure the vertical, horizontal and 
 * column space and if there should be divider lines shown.
 * 
 * 
...
public class LabelledComponentPanel extends JPanel {
	 ...
	 */
	public LabelledComponentPanel(int numberOfColumns, boolean verticalDivider, boolean horizontalDivider) {
	...
	public LabelledComponentPanel(int numberOfColumns, int hgap, int vgap, int colspace, 
			boolean labelAlignmentRight, boolean componentAlignmentRight, 
			boolean verticalDivider, boolean horizontalDivider) {
		this.hgap = hgap;
		this.vgap = vgap;
	 ...
	 * @param label
	 * @param comp
	 * @param fill Ignores left/right alignment and fills entire space with given 
	 * component
	 */
	public void addLabelComponent(int column, JLabel label, JComponent comp, boolean fill) {
		
		if (column > rows.length - 1) {
			throw new LabelledComponentPanelError("Column " + column + " out of bounds. " +
					"Number of available columns: " + rows.length);
		}
		
		if (horizontalDivider && rows[column] > 0) {
			//There is already a row -> add the spacers
			add(createSeparator(JSeparator.HORIZONTAL), createConstraints(LABEL, column, true));
			add(createSeparator(JSeparator.HORIZONTAL), createConstraints(COMPONENT, column, true));
			
			if (verticalDivider && column < rows.length - 1) {
				add(createSeparator(JSeparator.VERTICAL), createConstraints(SPACERVERTICAL, column, true));
			}
			
			rows[column]++;
		}
			
		JPanel p1 = new JPanel(new FlowLayout((labelAlignmentRight ? FlowLayout.RIGHT : FlowLayout.LEFT)));
		p1.add(label);
		...
		add(p1, createConstraints(LABEL, column, false));
		add(p2, createConstraints(COMPONENT, column, false));
