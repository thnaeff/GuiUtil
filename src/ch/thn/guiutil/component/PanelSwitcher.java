...
public class PanelSwitcher implements ActionListener {
	...
	/**
	 * Sets the dimension for all newly added buttons. If set to null, the button
	 ...
	 * 
	 * @return The SwitcherComponent of the component, or null if no component
	 * is visible
	 */
	 ...
	 * If switching to componentNr -1, the current panel will be hidden and no
	 * new panel will be shown.
	 * 
	...
	public boolean switchTo(int componentNr) {
		...
		if ((componentNr >= 0) && (sc.get(componentNr).getPanelTitle() != null)) {
			panelBorder = BorderFactory.createTitledBorder(BorderFactory.createLineBorder(cBorder), title);
