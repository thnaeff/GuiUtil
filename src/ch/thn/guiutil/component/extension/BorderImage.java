...
import ch.thn.guiutil.component.MouseCursorArea;
import ch.thn.guiutil.component.ToolTipArea;

/**
 * Draws images in the given component on top of everything. The speciality of 
 * this class is that the image can also be drawn outside of the components borders. 
 * The class will automatically extend the border (using an invisible outer border) 
 * to draw the image.<br>
 * The location of the images can be set with 
 * one of the constants<br>
 * <ul>
 ...
 * {@link #addImage(Image, int, int, int)} method gives the possibility to 
 * move the image out of the corner.<br>
 * <br>
 * ====== USAGE ======<br>
 * <br>
 ...
 *	<li>Add images to your {@link BorderImage} object, using the 
 *{@link #addImage(Image, int)} or {@link #addImage(Image, int, int, int)} method. </li>
 *	<li>Overwrite the paintBorder-method in your class. Add the call to {@link #paintImages(Graphics)} 
 *<b>after</b> the super-call super.paintBorder(g) (the images need to be painted on top 
 *of everything else, thus maybe paintChildren needs to be overwritten in case any 
 *components are painted over the image).</li>
 * </ul>
 * 
...
public class BorderImage implements AncestorListener, PropertyChangeListener, ComponentListener {
	...
	private BorderExtension borderExtension = null;
	...
	public BorderImage(JComponent component) {
		...
		borderExtension = new BorderExtension(component);
	 ...
	 * 	<li>SOUTH_WEST</li>
	 * </ul>
	 * The position of the image can be adjusted with the offset-parameters. If the 
	 * position of the image is outside of the component, the border will be extended 
	 * so that the image can be drawn.
	 * 
	 * 
	 * @param image
	 * @param position
	...
	/**
	 * Sets a tool tip message for the given image. The image needs to be added 
	 * before with one of the addImage-methods.
	 * 
	 * @param image
	 * @param toolTipString
	 */
	public void setToolTip(Image image, String toolTipString) {
		if (!images.containsKey(image)) {
			throw new ComponentExtensionError("Image " + image + " not found!");
		}
	...
	/**
	 * Sets a mouse cursor for the given image. The image needs to be added 
	 * before with one of the addImage-methods.
	 * 
	 * @param image
	 * @param cursor
	...
	/**
	 * Marks an image to be shown. The image needs to be added 
	 * before with one of the addImage-methods.
	 * 
	 * @param image
	 * @param show
	...
	/**
	 * Updates all the images and recalculates their position and the size of the 
	 * border
	 */
	private void update() {
		ExtendedBorder border = borderExtension.getExtendedBorder();
		int[] thickness = border.getThickness();
		
		componentWidth = (int)component.getSize().getWidth() 
				- thickness[BorderExtension.EAST] - thickness[BorderExtension.WEST];
		componentHeight = (int)component.getSize().getHeight() 
				- thickness[BorderExtension.NORTH] - thickness[BorderExtension.SOUTH];
...
//		component.getParent().validate();
	...
	/**
	 * Holds the added image and its position (with tool tip message and cursor 
	 * type)
	 * 
	 * @author Thomas Naeff (github.com/thnaeff)
	 *
	...
	private class AddedImage {
		...
		/**
		 * Recalculate the position of the image and the border needed (if the image 
		 * is outside of the component). The coordinates are measured from the top 
		 * left corner, thus since the borders might grow the coordinates have to 
		 * be adjusted after recalculating all the image positions using {@link #recalculate()}.
		 */
		...
		/**
		 * Adjusts the coordinates. Has to be called for all the {@link AddedImage}s 
		 * after the recalculation with {@link #recalculate()}.
		 */
		private void adjust() {
			x += left;
			y += top;
